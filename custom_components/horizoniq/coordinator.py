...
class HorizonIQCoordinator(DataUpdateCoordinator[HorizonIQSnapshot]):
    """Fetch and normalize HorizonIQ data for the integration."""
    ...
    async def _async_update_data(self) -> HorizonIQSnapshot:
        ...
        _LOGGER.debug(
            "Requesting HorizonIQ forecast for entry %s from %s",
        ...
        _LOGGER.debug(
            "Fetched HorizonIQ forecast for entry %s with %s periods",
            self._entry.entry_id,
    ...
    async def _fetch_payload(
        ...
        *,
        ...
        try:
                    ...
                    if response.status != HTTPStatus.OK:
                        ...
                        if response.status in {
                            ...
                            if (
                                ...
                                _LOGGER.info(
                                    "Retrying HorizonIQ forecast for entry %s after credential refresh",
                                    self._entry.entry_id,
                                )
                            ...
                            _LOGGER.info(
                                "HorizonIQ API returned unauthorized forecast "
            ...
            raise UpdateFailed("Timed out fetching HorizonIQ data") from err
        except ClientError as err:
            raise UpdateFailed(f"Error communicating with HorizonIQ API: {err}") from err

        if not isinstance(payload, dict):
            raise UpdateFailed("HorizonIQ API returned an unexpected payload shape")
        return payload
    ...
    async def _async_refresh_forecast_credentials(self) -> bool:
        ...
        try:
            ...
            _LOGGER.warning(
                "HorizonIQ forecast credential refresh failed for entry %s",
                self._entry.entry_id,
                exc_info=True,
            )
            return False
    ...
    async def _read_json_payload(self, response: ClientResponse) -> object:
        try:
            ...
            raise UpdateFailed("HorizonIQ API returned invalid JSON") from err
    ...
    def _build_request_headers(self) -> dict[str, str]:
        ...
        if self._forecast_device_id and self._forecast_device_token:
            headers[HEADER_FORECAST_DEVICE_ID] = self._forecast_device_id
            headers[HEADER_FORECAST_DEVICE_TOKEN] = self._forecast_device_token
        return headers
    ...
    def _update_cached_state(self, snapshot: HorizonIQSnapshot) -> bool:
        updated = False
    ...
    def _set_forecast_cadence_minutes(self, cadence_minutes: int | None) -> None:
        ...
        _LOGGER.debug(
            "Set HorizonIQ backend polling cadence for entry %s to %s minute(s)",
            self._entry.entry_id,
            resolved_minutes,
        )
    ...
    def _reset_forecast_cadence_minutes(self) -> None:
        ...
        _LOGGER.debug(
            "Reset HorizonIQ backend polling cadence for entry %s to default %s minute(s)",
            self._entry.entry_id,
            self._effective_forecast_cadence_minutes,
        )
