...
def to_json(snapshot: SimulationSnapshot) -> str:
    def encode(value: object) -> object:
        ...
        return value
    if snapshot.schema_version not in {1, 2, 3, SNAPSHOT_SCHEMA_VERSION}: raise ValueError("unsupported snapshot schema")
    return json.dumps(encode(snapshot),sort_keys=True,separators=(",",":"))
def from_json(value: str) -> SimulationSnapshot:
    raw=json.loads(value)
    if not isinstance(raw,dict) or raw.get("schema_version") not in {1, 2, 3, SNAPSHOT_SCHEMA_VERSION}: raise ValueError("unsupported snapshot schema")
    try:
