...
class MeshSolarOAuth2Implementation(LocalOAuth2ImplementationWithPkce):
    """Home Assistant PKCE implementation routed through the Mesh Solar portal."""
            ...
            async with session.get(runtime_config_url) as response:
                if response.status != 200:
                    raise OAuthRuntimeConfigError(
                        "Mesh Solar OAuth configuration is unavailable"
                    )
                try:
                    payload: Any = await response.json(content_type=None)
                except (ContentTypeError, ValueError) as err:
                    raise OAuthRuntimeConfigError(
                        "Mesh Solar OAuth configuration is invalid"
                    ) from err
    except TimeoutError as err:
        raise OAuthRuntimeConfigError(
            "Timed out loading Mesh Solar OAuth configuration"
        ) from err
    except ClientError as err:
        raise OAuthRuntimeConfigError(
            "Unable to load Mesh Solar OAuth configuration"
        ) from err

    if not isinstance(payload, dict):
        raise OAuthRuntimeConfigError("Mesh Solar OAuth configuration is invalid")
