...
class MeshSolarConfigFlow(
    config_entry_oauth2_flow.AbstractOAuth2FlowHandler, domain=DOMAIN
):
    """Handle Mesh Solar config and reauth flows."""
    ...
    async def _async_begin_oauth(self) -> ConfigFlowResult:
        ...
        self.flow_impl = MeshSolarOAuth2Implementation(
            self.hass,
            auth_domain=DOMAIN,
    ...
    def _bootstrap_client(self) -> MeshSolarBootstrapClient:
        if self._runtime is None or self._oauth_data is None:
            raise MeshSolarApiError("OAuth data is not available for bootstrap")

        return MeshSolarBootstrapClient(
...
def _entry_data_from_bootstrap(
    *,
    ...
    try:
        ...
        raise MeshSolarApiError("Entitled bootstrap response is incomplete") from err


def _no_subscription_schema(bootstrap: BootstrapData) -> vol.Schema:
    actions = {ACTION_CHECK_AGAIN: "Check Again", "subscribe": "Subscribe"}
...
def _no_subscription_placeholders(bootstrap: BootstrapData) -> dict[str, str]:
    return {
        ...
        "subscribe_url": bootstrap.subscribe_url or PORTAL_BILLING_URL,
...
def _is_valid_portal_connection_url(value: str) -> bool:
    ...
    if (
        ...
        or parsed.query
        or parsed.fragment
        or parsed.path.rstrip("/") != "/portal/ha-integration/connect"
    ):
        return False
