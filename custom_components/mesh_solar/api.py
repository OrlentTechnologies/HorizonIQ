...
class MeshSolarApiError(Exception):
    """Base API error with secret-free messages."""


class MeshSolarApiAuthError(MeshSolarApiError):
    """Raised when the backend rejects OAuth authentication."""


class MeshSolarApiTransientError(MeshSolarApiError):
    """Raised for temporary backend or network failures."""


class MeshSolarBootstrapClient:
    ...
    async def async_start_trial(self, *, installation_id: str) -> str:
        ...
        if not isinstance(payload, Mapping):
            raise MeshSolarApiError("Trial start returned an invalid response")
        token = payload.get("deviceToken")
        if not isinstance(token, str) or not token.strip():
            raise MeshSolarApiError("Trial start did not return a device token")
        return token.strip()
            ...
            async with asyncio.timeout(REQUEST_TIMEOUT_SECONDS):
                ...
                async with response:
                    if response.status == HTTPStatus.UNAUTHORIZED:
                        raise MeshSolarApiAuthError(
                            "Mesh Solar sign-in is no longer valid"
                        )
                    if response.status == HTTPStatus.TOO_MANY_REQUESTS or (
                        ...
                        raise MeshSolarApiTransientError(
                            "Mesh Solar service is temporarily unavailable"
                        )
                    if response.status >= HTTPStatus.BAD_REQUEST:
                        raise MeshSolarApiError(
                            f"Mesh Solar request failed with status {response.status}"
                        )
                    try:
                        return await response.json(content_type=None)
                    except (ContentTypeError, ValueError) as err:
                        raise MeshSolarApiError(
                            "Mesh Solar returned invalid JSON"
                        ) from err
        except TimeoutError as err:
            raise MeshSolarApiTransientError("Mesh Solar request timed out") from err
        except ClientError as err:
            raise MeshSolarApiTransientError(
                "Unable to communicate with Mesh Solar"
            ) from err
